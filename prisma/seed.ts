...
async function main() {
  ...
  console.log(`User seeded with ID: ${user.id}`);

  // --- Create Dummy Skills ---
  // ... (rest of the skills seeding code)

  // --- Create Dummy Jobs & Resources ---
  // ... (rest of the jobs/resources seeding code)

  // --- Link Skills to Jobs and Resources ---
  // ... (rest of the linking code)

  // --- NEW: Create a "SavedJob" record for the new user ---
  console.log("Seeding saved job...");
  // Find a job to save, for example the 'Frontend Developer' job
  const jobToSave = await prisma.job.findUnique({
    where: {
      title_companyName: {
        title: "Frontend Developer (React)",
        companyName: "Innovatech Solutions",
  ...
  if (jobToSave) {
    ...
    console.log("Saved job link seeded.");
  }
